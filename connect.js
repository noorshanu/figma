...
const Address = '0xeEC71376160C1b42f8ac7564e2B83f92c6013ce6'
const Abi = [
  {
    inputs: [
  ...
  {
    inputs: [
      { internalType: 'uint256', name: '_preSaleEndDate', type: 'uint256' },
    ],
  ...
  {
    inputs: [
      { internalType: 'uint256', name: '_publicSaleDate', type: 'uint256' },
    ],
...
async function enabler() {
  ...
  try {
    await ethereum.request({
      ...
      params: [{ chainId: '0x1' }], //change chainid with hexformat
    })
  } catch (switchError) {
    ...
    if (error.code === 4902) {
      try {
        ...
        }) //change the chainid with the correct one in hex format - same for rpcurl
      } catch (addError) {
        // handle "add" error
      }
    }
...
async function doing() {
  ...
  ciao.innerHTML = `${await signer.getAddress()}`

  // console.log("Account:", await signer.getAddress());
}

let ciao = document.querySelector('.ciao')
var yourString = ciao //replace with your string.
